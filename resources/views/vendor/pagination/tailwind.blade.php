@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between w-full">
        <div class="flex justify-between flex-1 md:hidden">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 ltr:mr-3 rtl:ml-3 text-xs font-medium text-gray-500 bg-white border border-gray-200 shadow-sm leading-5 rounded-sm tracking-wider hover:text-gray-500 focus:outline-none transition ease-in-out duration-150">
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" class="relative inline-flex items-center px-4 py-2 ltr:ml-3 rtl:mr-3 text-xs font-medium text-gray-500 bg-white border border-gray-200 shadow-sm leading-5 rounded-sm tracking-wider hover:text-gray-500 focus:outline-none transition ease-in-out duration-150">
        ...
        <div class="hidden md:flex-1 md:flex md:items-center md:justify-center">
            ...
            <div>
                <span class="relative z-0 flex space-x-1 rtl:space-x-reverse">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="relative inline-flex items-center justify-center text-xs font-medium w-8 h-8 leading-8 text-center border border-gray-100 rounded-full" aria-label="{{ __('pagination.previous') }}">
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="block w-8 h-8 leading-8 text-center text-white bg-indigo-500 border-indigo-500 rounded-full text-xs font-medium">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="block w-8 h-8 leading-8 text-center border border-gray-100 rounded-full text-xs font-medium" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
                    ...
                    {{-- Next Page Link --}}
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="relative inline-flex items-center justify-center text-xs font-medium w-8 h-8 leading-8 text-center border border-gray-100 rounded-full" aria-label="{{ __('pagination.next') }}">
