        ...
        <table class="w-full whitespace-nowrap">
            <thead class="bg-gray-200">
                <tr tabindex="0" class="focus:outline-none h-16 w-full text-sm leading-none text-gray-800 dark:text-white ">
                    ...
                    <th class="font-bold text-[10px] text-slate-500 uppercase tracking-wider text-center">{{ __('messages.t_price') }}</th>
                    <th class="font-bold text-[10px] text-slate-500 uppercase tracking-wider text-center">{{ __('messages.t_visits') }}</th>
                    <th class="font-bold text-[10px] text-slate-500 uppercase tracking-wider text-center">{{ __('messages.t_sales') }}</th>
                    <th class="font-bold text-[10px] text-slate-500 uppercase tracking-wider text-center">{{ __('messages.t_in_queue') }}</th>
                    <th class="font-bold text-[10px] text-slate-500 uppercase tracking-wider text-center">{{ __('messages.t_rating') }}</th>
                    <th class="font-bold text-[10px] text-slate-500 uppercase tracking-wider text-center">{{ __('messages.t_status') }}</th>
                    <th class="font-bold text-[10px] text-slate-500 uppercase tracking-wider text-center">{{ __('messages.t_created_at') }}</th>
            ...
            <tbody class="w-full">
                ...
                @foreach ($gigs as $gig)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white hover:bg-gray-100 border-b border-t border-gray-100" wire:key="reports-{{ $gig->id }}">
                        ...
                        <td class="ltr:pl-4 ltr:pr-10 rtl:pr-4 rtl:pl-10 w-[40%]">
                            <div class="flex items-center">
                                ...
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <a href="{{ url('service', $gig->slug) }}" target="_blank" class="font-medium text-xs hover:text-indigo-600 truncate pb-1.5 block max-w-xs">{{ $gig->title }}</a>
                        ...
                        <td class="text-center">
                            <p class="text-xs font-black text-black font-[Lato]">@money($gig->price, settings('currency')->code, true)</p>
                        </td>

                        {{-- Visits --}}
                        <td class="text-center">
                            <p class="text-xs font-black text-black font-[Lato]">{{ $gig->counter_visits }}</p>
                        </td>

                        {{-- Sales --}}
                        <td class="text-center">
                            <p class="text-xs font-black text-black font-[Lato]">{{ $gig->counter_sales }}</p>
                        </td>

                        {{-- In queue --}}
                        <td class="text-center">
                            <p class="text-xs font-black text-black font-[Lato]">{{ $gig->orders_in_queue }}</p>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="relative inline-block text-left">
                                <div>
                                    <button data-dropdown-toggle="table-options-dropdown-{{ $gig->id }}" type="button" class="inline-flex justify-center items-center rounded-full h-8 w-8 bg-white hover:bg-gray-50 focus:outline-none focus:ring-0" id="menu-button" aria-expanded="true" aria-haspopup="true">
                                ...
                                <div id="table-options-dropdown-{{ $gig->id }}" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="py-1" role="none">

                                        {{-- Publish gig --}}
                                        @if ($gig->status === 'pending')
                                            <button wire:key="option-publish-{{ $gig->id }}" x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_publish_this_gig') }}') ? $wire.publish('{{ $gig->id }}') : ''" wire:loading.attr="disabled" wire:target="publish('{{ $gig->id }}')" type="button" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
                                        ...
                                        {{-- Delete report --}}
                                        <button wire:key="option-delete-{{ $gig->id }}" x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this') }}') ? $wire.delete('{{ $gig->id }}') : ''" wire:loading.attr="disabled" wire:target="delete('{{ $gig->id }}')" type="button" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
