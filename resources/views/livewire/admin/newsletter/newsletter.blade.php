        ...
        <table class="w-full whitespace-nowrap">
            <thead class="bg-gray-200">
                <tr tabindex="0"
                    ...
                    <th class="font-bold text-[10px] text-slate-500 uppercase tracking-wider ltr:text-left ltr:pl-4 rtl:text-right rtl:pr-4">{{ __('messages.t_ip_address') }}</th>
            ...
            <tbody class="w-full">
                ...
                @foreach ($lists as $list)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white hover:bg-gray-100 border-b border-t border-gray-100" wire:key="lists-{{ $list->id }}">
                        ...
                        {{-- Ip address --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="https://ip-api.com/#{{ $list->ip_address }}" target="_blank" class="font-medium text-xs text-indigo-600 hover:text-indigo-700">{{ $list->ip_address }}</a>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="relative inline-block text-left">
                                <div>
                                    <button data-dropdown-toggle="table-options-dropdown-{{ $list->id }}" type="button" class="inline-flex justify-center items-center rounded-full h-8 w-8 bg-white hover:bg-gray-50 focus:outline-none focus:ring-0" id="menu-button" aria-expanded="true" aria-haspopup="true">
                                ...
                                <div id="table-options-dropdown-{{ $list->id }}" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="py-1" role="none">

                                        {{-- Resend verification --}}
                                        @if ($list->status === 'pending')
                                            <button wire:click="resend('{{ $list->id }}')" wire:loading.attr="disabled" wire:target="resend('{{ $list->id }}')" type="button" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
                                        ...
                                        {{-- Delete --}}
                                        <button x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this') }}') ? $wire.delete('{{ $list->id }}') : ''" wire:loading.attr="disabled" wire:target="delete('{{ $list->id }}')" type="button" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
