<div x-data="window.XnbzELJbXoSEFED" x-init="init()" x-cloak>
    ...
    <div class="group -m-2 p-2 flex items-center cursor-pointer" @click="open = !open">
        ...
        <span class="ltr:ml-2 rtl:mr-2 text-sm font-medium text-gray-700 group-hover:text-gray-800">{{ count($cart) }}</span>
    </div>
                    ...
                    <div class="flex h-full flex-col bg-white shadow-xl">
                        <div class="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                            <div class="flex items-start justify-between rtl:flex-row-reverse">
                                <h2 class="text-base tracking-wide font-extrabold text-gray-900"> {{ __('messages.t_my_shopping_cart') }} </h2>
                                                        ...
                                                        <div>
                                                            <div class="flex justify-between text-sm font-medium text-gray-900">
                                                                <h3 class="ltr:pr-2 rtl:pl-2 truncate w-32 lg:w-auto">
                                                                    <a href="{{ url('service', $item['gig']['slug']) }}" target="_blank" class="hover:text-indigo-600 font-bold truncate w-52 block"> {{ $item['gig']['title'] }} </a>
                                                                </h3>
                                                                <p class="ltr:ml-5 rtl:mr-5 font-[Lato] font-black text-gray-500 pt-0.5">@money($this->itemTotalPrice($item['id']), settings('currency')->code, true)</p>
                                                            </div>
                                                        </div>
                                                        <div class="flex flex-1 items-end justify-between text-xs">
                                                            <p class="text-gray-500">{{ __('messages.t_quantity_number', ['number' => $item['quantity']]) }}</p>
                                                            <div class="flex">
                            ...
                            @else
                                <div class="py-14 px-6 text-center sm:px-14 border-2 border-dashed mt-24">
                                    ...
                                    <p class="mt-4 font-semibold text-gray-900 text-base">{{ __('messages.t_ur_cart_is_empty') }}</p>
                                    <p class="mt-2 text-gray-500 text-sm font-normal">{{ __('messages.t_ur_cart_is_empty_subtitle') }}</p>
                                </div>
                            @endif
                        ...
                        @if (count($cart))
                            <div class="border-t border-gray-200 py-6 px-4 sm:px-6">
                                <div class="flex justify-between text-base font-medium text-gray-900">
                                    ...
                                    <p class="font-black font-[Lato]">@money($this->total(), settings('currency')->code, true)</p>
                                </div>
                                <p class="mt-0.5 text-sm text-gray-500">{{ __('messages.t_tax_fees_calculated_at_checkout') }}</p>
                                <div class="mt-6">
                                    <a href="{{ url('cart') }}" class="flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-indigo-700">{{ __('messages.t_view_cart') }}</a>
                                </div>
                                <div class="mt-6 flex justify-center text-center text-sm text-gray-500">
                                    <p>
                                        {{ __('messages.t_or') }} <button type="button" class="font-medium text-indigo-600 hover:text-indigo-500" @click="open = false">{{ __('messages.t_continue_shopping') }}<span aria-hidden="true"> →</span></button>
                                    </p>
                                </div>
                            </div>
                        @endif
