            ...
            <div class="divide-y divide-gray-200 lg:grid lg:grid-cols-12 lg:divide-y-0 lg:divide-x rtl:divide-x-reverse">
                ...
                <div class="divide-y divide-gray-200 lg:col-span-9">
                    ...
                    <div class="py-6 px-4 sm:p-6 lg:pb-8">
                        ...
                        {{-- Section header --}}
                        <div class="mb-14">
                            <h2 class="text-base leading-6 font-bold text-gray-900">{{ __('messages.t_request_refund') }}
                            </h2>
                            <p class="mt-1 text-sm text-gray-500">{{ __('messages.t_request_refund_subtitle') }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-12 md:gap-x-8 gap-y-8 mb-6">
                            ...
                            <div class="col-span-12 mb-10">
                                <div class="bg-white border-gray-200 border-dashed border-2 rounded-md">
                                    <div class="py-6 px-4 sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:p-8">
                                        ...
                                        <div class="mt-6 lg:mt-0 lg:col-span-5">
                                            <dl class="grid grid-cols-2 gap-x-6 text-sm">
                                                <div>
                                                    <dt class="font-medium text-gray-900">{{ __('messages.t_amount') }}</dt>
                                                    <dd class="mt-3 text-gray-500">
                                                        <span class="block">@money($item->total_value, settings('currency')->code, true)</span>
                                                    </dd>
                                                </div>
                                                <div>
                                                    <dt class="font-medium text-gray-900">{{ __('messages.t_quantity') }}</dt>
                                    ...
                                    <div class="border-t-2 border-dashed border-gray-200 py-6 px-4 sm:px-6 lg:p-8">
                                        <h4 class="sr-only">Status</h4>
                                        <p class="text-sm font-medium text-gray-900">{{ __('messages.t_expected_delivery_date_on_date', ['date' => format_date($item->expected_delivery_date, 'F j, Y h:i A')]) }}</p>
                                        <div class="mt-6" aria-hidden="true">
                                            ...
                                            <div class="hidden sm:grid grid-cols-3 text-sm font-medium text-gray-600 mt-6">
                                                <div class="text-indigo-600">{{ __('messages.t_order_placed') }}</div>
                                                <div class="text-center {{ in_array($item->status, ['proceeded', 'delivered']) ? 'text-indigo-600' : '' }}">{{ __('messages.t_processing') }}</div>
                                                <div class="ltr:text-right rtl:text-left {{ $item->status === 'delivered' ? 'text-indigo-600' : '' }}">{{ __('messages.t_delivered') }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
