        ...
        <header class="bg-gray-50 py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    {{-- Title --}}
                    <h1 class="mt-2 text-xl font-bold leading-7 text-gray-900 sm:text-2xl sm:truncate">{{ __('messages.t_make_withdrawal') }}</h1>
                ...
                <div class="mt-5 flex xl:mt-0 xl:ltr:ml-4 xl:rtl:mr-4">
                    ...
                    <span class="hidden sm:block ltr:ml-3 rtl:mr-3">
                        <a href="{{ url('seller/withdrawals') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-sm shadow-sm text-[13px] font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-purple-500">
            ...
            @if ($can_withdraw)
                ...
                <div class="grid grid-cols-12 md:gap-x-6 gap-y-6">
                    ...
                    <div class="col-span-12">
                        <label for="text-input-component-id-email" class="block text-xs font-medium tracking-wide text-gray-700">{{ __('messages.t_paypal_email') }}</label>
                        <div class="mt-2 relative">
                            <input type="email" placeholder="{{ __('messages.t_enter_paypal_email') }}" value="{{ $email }}" disabled readonly id="text-input-component-id-email" class="block w-full text-xs rounded border-2 pr-10 py-3 pl-3 font-normal border-gray-200 placeholder-gray-400 focus:ring-indigo-500 focus:border-indigo-500 cursor-not-allowed">
                    ...
                    {{-- Amount requested --}} 
                    <div class="col-span-12">
                        <label for="text-input-component-id-amount" class="block text-xs font-medium tracking-wide {{ $errors->first('amount') ? 'text-red-600' : 'text-gray-700' }}">{{ __('messages.t_amount_to_withdraw') }}</label>
                        <div class="mt-2 relative">
                            <input type="text" placeholder="0.00" wire:model.defer="amount" id="text-input-component-id-amount" class="block w-full text-xs rounded border-2 pr-10 py-3 pl-3 font-normal {{ $errors->first('amount') ? 'border-red-500 text-red-600 placeholder-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-200 placeholder-gray-400 focus:ring-indigo-500 focus:border-indigo-500' }}">
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <i class="mdi mdi-cash-multiple {{ $errors->first('amount') ? 'text-red-400' : 'text-gray-400' }}"></i>
                            </div>
                        </div>
                        <p class="mt-1 text-xs text-gray-500">{{ __('messages.t_available_balance_amount', ['amount' => money(auth()->user()->balance_available, settings('currency')->code, true)]) }}</p>
                        @error('amount')
