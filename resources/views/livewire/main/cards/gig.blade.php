<div class="gig-card" x-data="window._{{ $gig->uid }}" dir="{{ config()->get('direction') }}">
    <div class="bg-white rounded-md shadow-sm ring-1 ring-gray-200">
        ...
        <div class="px-4 pb-2 mt-4">
            ...
            {{-- Title --}}
            <a href="{{ url('service', $gig->slug) }}" class="gig-card-title text-gray-800 hover:text-indigo-600 mb-1">
                {{ $gig->title }}
            </a>
            ...
            <div class="flex items-center" wire:ignore>
                ...
                {{-- Rating --}}
                @if ($gig->rating == 0)
                    <div class="font-[Lato] text-[13px] tracking-widest text-amber-500 ltr:ml-1 rtl:mr-1 font-black">
                        {{-- {{ __('messages.t_n_a') }} --}} 0.0
                    </div>
                @else
        ...
        <div class="px-3 py-3 bg-[#fdfdfd] border-t border-gray-50 text-right sm:px-4 rounded-b-md flex justify-between">
            ...
            <div class="gig-card-price">
                ...
                <span class="font-[Lato] ltr:text-right rtl:text-left">{{$gig->price}} DH</span>
            </div>
