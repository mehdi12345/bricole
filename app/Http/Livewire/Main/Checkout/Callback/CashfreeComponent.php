...
{    
    ...
    public function mount()
    {
        ...
        $this->cashfree_order_id = $billing_info['cashfree_order_id'];

        // Get payment details
        $paymentDetails          = new CashfreeOrder();

        // Get order details
        $details                 = $paymentDetails->getDetails($billing_info['cashfree_order_id']);

        // Check if payment succeeded
        if ( is_object($details) && $details->status === "OK" && $details->details && $details->details->orderStatus === "PAID")
        {
           ...
           $this->place($details);    
            ...
            // Payment failed
            return redirect('checkout')->with('error', __('messages.t_error_cashfree_payment_failed'));
    ...
    public function place($payment)
    {
        try {
            ...
            $invoice->payment_method = 'cashfree';
            $invoice->payment_id     = $this->cashfree_order_id;
